...
@pytest.mark.django_db
def test_import_unresolved_tracks_includes_artist_without_mbid(settings, tmp_path):
    settings.YOUTUBE_FALLBACK_ENABLE = True
    video_dir = tmp_path / "videos"
    audio_dir = tmp_path / "audio"
    video_dir.mkdir(parents=True, exist_ok=True)
    ...
    pl = PlaylistFactory()
    artist = ArtistFactory(mbid=None)
    ti = TrackItem.objects.create(
        playlist=pl,
        video_id="mbidless01",
        title="MBIDless Artist - Song",
        channel_title="MBIDless Artist - Topic",
        artist_name_guess="MBIDless Artist",
        artist=artist,
    )
